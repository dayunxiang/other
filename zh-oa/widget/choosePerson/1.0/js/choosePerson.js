...
choosePerson.init = function(options){
    ...
    choosePerson.search();
    if(choosePerson.opts.initDataId && choosePerson.opts.initDataName && choosePerson.opts.initDataParent){
...
choosePerson.addDiyDom = function(treeId, treeNode){
    if (treeNode.isParent && choosePerson.opts.selectAllBtn) {
        ...
        var _ahtml = '<a href="javascript:;" id="selectBtn'+ treeNode.tId +'">全选</a>';
        aObj.append(_ahtml);
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for(var i=0; i<data.Result.length; i++){
                    Html+='<tr data-parent="false" data-name="'+data.Result[i].CNAME+'" data-id="'+data.Result[i].USERID+'">' +
                            '<td class="c-t-center"><span class="c-nowrap">'+data.Result[i].CNAME+'</span></td>' +
                            '<td><span class="c-nowrap">'+data.Result[i].ORGNAME+'</span></td>' +
                        '</tr>';
                }
...
'<div class="c-main clearfix c-hide" id="choosePerson-dialog">'+
    '<div class="fl w350 h400 c-border c-hidden">'+
        '<div class="c-position-r c-border-b" id="searchShow">'+
            '<div class="c-666 ptb10 plr15 c-line-h1">'+
                '<input type="text" placeholder="搜索部门人名或用户凭证..." class="width-90 c-f14 list-search-input">'+
    '<i class="iconfont c-position-a c-position-rt10 mr30 c-cursor-p c-hide list-search-clear" title="清空">&#xe68b;</i>'+
        ...
        '</div>'+
    '</div>'+
    '<div class="fr w350 h400 c-border c-hidden">'+
...
choosePerson.initHtmlWrap = function () {
    if($('#choosePerson-dialog').length<=0){
