		...
		init : function(options, name) {
			...
			var gridObject = {
				...
				constructGridPageBar : function(){
					...
					//设置工具条初始内容
					var gridStatus = document.createElement('div');
					if(gridReflectionObj.exhibitDatas==null||gridReflectionObj.exhibitDatas.length<=0){
						gridStatus.className = 'dlshouwen-grid-pager-status text-primary pager-status';
					...
					operations.className = 'pagination pagination-sm dlshouwen-grid-pager-button pager-wrap';
				...
				constructGridToolBar : function(){
					...
					var gridReflectionObj = this;
					//清空工具条
					$('#'+gridReflectionObj.option.toolbarContainer).html('<div class="dlshouwen-grid-pager inner-wrap clearfix"></div>');
					//如果为空则不做任何操作
...
(function($) {
	$.fn.dlshouwen.grid.defaultOptions = {
		grid : {
			...
			tableStyle : '',
			tableClass : 'dlshouwen-grid tab-1',
			showHeader : true,
